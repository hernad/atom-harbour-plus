...
describe "build", ->
  ...
  describe "when syntax check on save is enabled", ->
    ...
    beforeEach ->
      ...
      filePath = path.join(directory, "src", "github.com", "testuser", "example", "harbour-plus.go")
      testFilePath = path.join(directory, "src", "github.com", "testuser", "example", "harbour-plus_test.go")
  ...
  describe "when files are opened outside a gopath", ->
    ...
    beforeEach ->
      ...
      atom.config.set("harbour-plus.goPath", otherdirectory)
      atom.config.set("harbour-plus.syntaxCheckOnSave", true)
      filePath = path.join(directory, "src", "github.com", "testuser", "example", "harbour-plus.go")
      testFilePath = path.join(directory, "src", "github.com", "testuser", "example", "harbour-plus_test.go")
      fs.writeFileSync(filePath, '')
    ...
    it "displays warnings about the gopath, but still displays errors", ->
      ...
      runs ->
        ...
        dispatch.once 'dispatch-complete', =>
          ...
          expect(dispatch.messages[0]?.msg).toBe 'Warning: GOPATH [' + otherdirectory + '] does not contain a "src" directory - please review http://harbourlang.org/doc/code.html#Workspaces'
