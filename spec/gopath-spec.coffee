...
describe "gopath", ->
  ...
  describe "when syntax check on save is enabled and goPath is set", ->
    beforeEach ->
      atom.config.set("harbour-plus.goPath", directory)
      atom.config.set("harbour-plus.syntaxCheckOnSave", true)
      filePath = path.join(directory, "wrongsrc", "github.com", "testuser", "example", "harbour-plus.go")
      ...
      waitsForPromise ->
        atom.packages.activatePackage('harbour-plus')

      runs ->
        dispatch = atom.packages.getLoadedPackage('harbour-plus').mainModule.dispatch
    ...
    it "displays a warning for a GOPATH without 'src' directory", ->
      ...
      runs ->
        ...
        dispatch.once 'dispatch-complete', =>
          ...
          expect(dispatch.messages[0]?.msg).toBe "Warning: GOPATH [" + directory + "] does not contain a \"src\" directory - please review http://harbourlang.org/doc/code.html#Workspaces"
    ...
    it "displays warnings for an unset GOPATH", ->
      ...
      runs ->
        ...
        dispatch.once 'dispatch-complete', =>
          ...
          expect(dispatch.messages[0]?.column).toBe false
          expect(dispatch.messages[0]?.line).toBe false
          expect(dispatch.messages[0]?.msg).toBe "Warning: GOPATH is not set – either set the GOPATH environment variable or define the Go Path in harbour-plus package preferences"
          expect(dispatch.messages[0]?.type).toBe 'warning'
          done = true
        buffer.save()
